...
export function SiteHeader() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-pink/15 bg-background/88 backdrop-blur-xl">
      <div className="mx-auto flex min-h-16 max-w-7xl items-center justify-between gap-2 px-4 py-2 sm:gap-4 sm:px-5 sm:py-3 md:px-8">
        <a href="#top" className="flex min-h-11 items-center gap-2.5">
          ...
          />
          <span className="font-display text-base font-bold tracking-[0.04em] text-foreground sm:text-lg">
            brand<span className="text-pink">cultura</span>
          </span>
          <span className="agency-glitch agency-glitch--small ml-2 hidden xl:inline" data-text="AGENCY">AGENCY</span>
        </a>
        <nav className="hidden flex-1 items-center justify-center gap-7 px-5 xl:flex 2xl:gap-10" aria-label={copy.nav.aria}>{labels.map((label, i) => <a key={HREFS[i]} href={HREFS[i]} className="label-mono tracking-[0.24em] text-muted-foreground transition-colors hover:text-foreground">{label}</a>)}</nav>
        <div className="flex shrink-0 items-center gap-3">
          ...
          <a href="#kontakt" className="hidden border border-lime/60 bg-lime px-4 py-2 text-sm font-semibold text-lime-foreground transition-colors hover:bg-foreground sm:block">{copy.nav.start}</a>
        </div>
      </div>
    </header>
  )
