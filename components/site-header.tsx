...
export function SiteHeader() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-pink/15 bg-background/88 backdrop-blur-xl">
      <a
        href="/#kontakt"
        className="sr-only focus:not-sr-only focus:absolute focus:left-3 focus:top-3 focus:z-[60] focus:bg-lime focus:px-3 focus:py-2 focus:text-sm focus:font-semibold focus:text-lime-foreground"
      >
        {copy.nav.skip}
      </a>
      <div className="mx-auto flex min-h-16 max-w-7xl items-center justify-between gap-2 px-3 py-2 sm:gap-4 sm:px-5 sm:py-3 md:px-8">
        <a href="/#top" className="flex min-h-11 min-w-0 items-center gap-2.5" onClick={close}>
          <Image
            ...
            height={42}
            priority
            className="h-9 w-9 shrink-0 object-cover mix-blend-screen sm:h-10 sm:w-10"
          />
          <span className="truncate font-display text-base font-bold tracking-[0.04em] text-foreground sm:text-lg">
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          <a
            href="/#kontakt"
            className="border border-lime/60 bg-lime px-3 py-2 text-sm font-semibold text-lime-foreground transition-colors hover:bg-foreground"
          >
            {copy.nav.start}
          </a>
          <button
            type="button"
            className="min-h-11 min-w-11 border border-border px-2 font-mono text-[11px] uppercase text-foreground md:hidden"
            aria-expanded={open}
...
function LocaleButtons({ locale, setLocale }: { locale: Locale; setLocale: (locale: Locale) => void }) {
  return (['de', 'en', 'ru'] as Locale[]).map((item) => (
    <button
      ...
      className={`min-h-11 min-w-10 flex-1 px-2 py-2 font-mono text-[11px] uppercase transition-colors ${
        locale === item ? 'bg-lime text-lime-foreground' : 'text-muted-foreground hover:text-foreground'
      }`}
    >
      {item}
    </button>
