  ...
  return (
    <section id="top" className="mx-auto max-w-6xl px-4 pb-10 pt-8 sm:px-5 sm:pb-14 sm:pt-12 md:px-8 md:pb-20 md:pt-16">
      ...
      <div className="flex flex-wrap items-center gap-x-5 gap-y-2 border-b border-border pb-4">
        <span className="label-mono text-gold">{t.eyebrow}</span>
      ...
      <div className="grid gap-8 pt-10 md:grid-cols-[1.15fr_0.85fr] md:items-end md:gap-10 md:pt-16">
        <Reveal>
          <p className="label-mono mb-6 text-cobalt">01 / sound identity</p>
        ...
        <Reveal delay={120} className="min-w-0">
          <div className="premium-panel flex h-full flex-col justify-between gap-5 p-4 sm:p-5">
            <div className="flex items-center justify-between">
              <span className="label-mono text-gold">{t.signal}</span>
            ...
            <div className="flex h-24 items-end gap-1 sm:h-28" aria-hidden="true">
              {EQ_BARS.map((_, i) => (
                <span
                  key={i}
                  className={`eq-bar flex-1 rounded-sm ${i % 5 === 0 ? 'bg-cobalt' : 'bg-gold/80'}`}
        ...
        <div className="grid gap-8 md:grid-cols-[1fr_auto] md:items-end md:gap-12">
          <dl className="grid grid-cols-3 gap-4 sm:gap-8">
            {[['20 Hz - 20 kHz', t.frequency], ['∞', t.titleEnd], ['2026', t.eyebrow]].map(([value, label]) => (
              <div key={value} className="border-l border-gold/50 pl-3 sm:pl-4">
          ...
          <div className="flex flex-col gap-3 sm:flex-row">
            <a href="#kontakt" className="flex min-h-12 items-center justify-center bg-gold px-6 py-3 text-sm font-semibold text-background transition-transform hover:-translate-y-0.5 hover:bg-foreground">{t.discuss}</a>
            <a href="#analyse" className="flex min-h-12 items-center justify-center border border-border px-6 py-3 text-sm font-semibold text-foreground transition-colors hover:border-cobalt hover:text-cobalt">{t.spectrum}</a>
          </div>
        </div>
      </Reveal>
    </section>
