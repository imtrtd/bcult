      ...
      <div className="mx-auto max-w-6xl px-4 py-14 sm:px-5 sm:py-16 md:px-8 md:py-24">
        <Reveal>
          <div className="flex items-center gap-3">
            <span className="node-pulse h-2 w-2 bg-cobalt" aria-hidden />
        ...
        <div className="mt-9 grid grid-cols-1 gap-3 sm:mt-12 lg:grid-cols-12">
          <Reveal className="lg:col-span-7">
            <div className="image-stage signal-scan group relative aspect-[16/10] overflow-hidden border border-gold/20">
              <Image
              ...
              <span className="label-mono absolute bottom-4 left-4 z-10 border border-gold/40 bg-background/75 px-3 py-2 text-gold backdrop-blur-md">
                spectrogram → form
              </span>
            </div>
          </Reveal>
          ...
          <div className="flex flex-col gap-3 lg:col-span-5">
            {t.cards.map((card, index) => (
              <Reveal key={card[0]} delay={index * 100} className="flex-1">
                <div className="premium-panel group flex h-full flex-col gap-3 p-5 transition-colors hover:border-gold sm:p-6">
                  <span className="label-mono text-muted-foreground transition-colors group-hover:text-gold">
                    {card[0]}
                  </span>
                ...
                <span key={dup} className="inline-flex items-center">
                  {ticker.map((word, i) => (
                    <span key={`${dup}-${i}`} className="label-mono inline-flex items-center text-muted-foreground">
                      ...
                      <span className="mx-5 h-1 w-1 rounded-full bg-cobalt/70" />
                    </span>
                  ))}
                </span>
              ))}
        ...
        <div className="mt-3 grid grid-cols-1 gap-3 md:grid-cols-2">
          {t.pillars.map((p, index) => (
            <Reveal key={p[0]} delay={index * 100}>
              <div className="premium-panel group flex min-h-full flex-col gap-4 p-5 transition-colors hover:border-gold sm:p-6 md:p-10">
                <div className="flex items-center justify-between">
                  <span className="label-mono text-gold">{p[0]}</span>
                  <span
                    className="display text-4xl font-extrabold text-border transition-colors group-hover:text-cobalt/40 md:text-5xl"
                    aria-hidden
                  >
